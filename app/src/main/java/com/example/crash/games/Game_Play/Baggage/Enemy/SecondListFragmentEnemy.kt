...
import com.example.crash.databinding.FragmentSecondListEnemyBinding
...
class SecondListFragmentEnemy : Fragment() {
    lateinit var baggageBinding: FragmentSecondListEnemyBinding
    ...
    override fun onCreateView(
        ...
        dataGames.BlockInBaggageEnemy2.value?.let { blocks.addAll(it) }
        baggageBinding= FragmentSecondListEnemyBinding.inflate(inflater)
                    ...
                    if (blocks.size != 0) {
                        for (block in blocks.reversed()) {
                            if (block!!.containsPoint(event.x.toInt(), event.y.toInt())) {
                                ...
                                dataGames.BlockInBaggageEnemy2.value = blocks
    ...
    fun check_in_baggage(y:Int,view: View){
        ...
        if(y>view.height){
            activity?.supportFragmentManager?.beginTransaction()?.setCustomAnimations(R.anim.slide_out,R.anim.enemy_slide_in)?.remove(this)?.commit()
        }
    }
    ...
    companion object {
        @JvmStatic
        fun newInstance() = SecondListFragmentEnemy()
    }
}